...
########################################################################################################################
# DEFAULT CONFIG VALUES: These values will be used when nothing has been specified in conf/aura-conf.yaml
...
DEFAULT_JOBM_HEAP_MB=256                            # Java heap size for the Workmanager (in MB)
...
########################################################################################################################
# CONFIG KEYS: The default values can be overwritten by the following keys in conf/aura-conf.yaml
########################################################################################################################

KEY_JOBM_HEAP_MB="workmanager.heap.mb"
...
YAML_CONF=${AURA_CONF_DIR}/aura-conf.yaml
...
# Define AURA_JM_HEAP if it is not already set
if [ -z "${AURA_JM_HEAP}" ]; then
    AURA_JM_HEAP=$(readFromConfig ${KEY_JOBM_HEAP_MB} ${DEFAULT_JOBM_HEAP_MB} ${YAML_CONF})
fi

# Define AURA_TM_HEAP if it is not already set
if [ -z "${AURA_TM_HEAP}" ]; then
    AURA_TM_HEAP=$(readFromConfig ${KEY_TASKM_HEAP_MB} ${DEFAULT_TASKM_HEAP_MB} ${YAML_CONF})
fi
...
# DO NOT USE FOR MEMORY SETTINGS! Use conf/aura-conf.yaml with keys
