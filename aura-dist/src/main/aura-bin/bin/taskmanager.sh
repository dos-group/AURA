...
log=$AURA_LOG_DIR/aura-$AURA_IDENT_STRING-taskmanager-$HOSTNAME.log
out=$AURA_LOG_DIR/aura-$AURA_IDENT_STRING-taskmanager-$HOSTNAME.out
pid=$AURA_PID_DIR/aura-$AURA_IDENT_STRING-taskmanager.pid
...
case $STARTSTOP in
    ...
    (start)
        ...
        $JAVA_RUN $JVM_ARGS $AURA_OPTS $log_setting -classpath $AURA_TM_CLASSPATH de.tuberlin.aura.taskmanager.TaskManager -configDir $AURA_CONF_DIR > "$out" 2>&1 < /dev/null &
        echo $! > $pid
    ;;
