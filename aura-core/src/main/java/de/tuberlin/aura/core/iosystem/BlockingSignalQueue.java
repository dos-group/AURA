 ...
 * queue is that element that has been on the queue the longest time. The <em>tail</em> of the queue
 * is that element that has been on the queue the shortest time. New elements are inserted at the
...
public class BlockingSignalQueue<E> extends AbstractQueue<E> implements BlockingQueue<E>, java.io.Serializable, BufferQueue<E> {
     ...
     * 
     * Whenever an element is enqueued, the putLock is acquired and count updated. A subsequent
     * reader guarantees visibility to the enqueued Node by either acquiring the putLock (via
     * fullyLock) or by acquiring the takeLock, and then reading n = count.get(); this gives
     ...
     */
    public BlockingSignalQueue(int capacity) {
        if (capacity <= 0)
     ...
     */
    public BlockingSignalQueue(Collection<? extends E> c) {
        this(Integer.MAX_VALUE);
        final ReentrantLock putLock = this.putLock;
     ...
     * {@code false} if this queue is full. When using a capacity-restricted queue, this method is
     * generally preferable to method {@link java.util.concurrent.BlockingQueue#add add}, which can
    ...
    private class Itr implements Iterator<E> {
         ...
         * hasNext() reports true, we will still have it to return even if lost race with a take
         * etc.
         */
