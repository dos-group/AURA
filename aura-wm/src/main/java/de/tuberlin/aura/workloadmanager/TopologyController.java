...
public class TopologyController extends EventDispatcher {
    ...
    private final class TaskStateConsensusEventHandler extends EventHandler {
        ...
        public TaskStateConsensusEventHandler() {
            ...
            this.readyConsensus = new TaskStateConsensus(EnumSet.of(TaskState.TASK_STATE_READY),
                    EnumSet.of(TaskState.TASK_STATE_FAILURE), topology);

            this.finishedConsensus = new TaskStateConsensus(EnumSet.of(TaskState.TASK_STATE_FINISHED),
                    EnumSet.of(TaskState.TASK_STATE_FAILURE), topology);

            this.failureConsensus = new TaskStateConsensus(EnumSet.of(TaskState.TASK_STATE_FINISHED,
                    TaskState.TASK_STATE_FAILURE, TaskState.TASK_STATE_CANCELED),
                    null, topology);
        }
        ...
        private synchronized void handleMonitoredTaskStateEvent(final IOEvents.MonitoringEvent event) {
            ...
            if (topology.monitoringProperties.contains(AuraTopology.MonitoringType.TASK_MONITORING)) {
                ioManager.sendEvent(topology.machineID, new IOEvents.MonitoringEvent(event)); // We need a copy of the event....
            }
            ...
            if (readyConsensus.voteFail(event.taskStateUpdate)) {
                ...
                // Some task failed, we cancel all other tasks. At the moment this is very conservative strategy,
            ...
            if (finishedConsensus.voteSuccess(event.taskStateUpdate)) {
                ...
                // Every task is successfully finished, then we move on to finish the complete topology.
		...
		private void handleTopologyStateTransition(final TopologyStateTransitionEvent event) {
            ...
            if (topology.monitoringProperties.contains(AuraTopology.MonitoringType.TOPOLOGY_MONITORING)) {
                ...
                final IOEvents.MonitoringEvent.TopologyStateUpdate topologyStateUpdate =
                        new IOEvents.MonitoringEvent.TopologyStateUpdate(topology.name, oldState, nextState, event.transition, currentTime - timeOfLastStateChange);
			...
			switch (state) {
                ...
                case TOPOLOGY_STATE_UNDEFINED: {
                    throw new IllegalStateException("topology " + topology.name + " [" + topology.topologyID + "] from state "
                        + oldState + " to " + state + " is not defined" + " [" + event.transition + "]");
                }
			}
		...
		TopologyBreadthFirstTraverser.traverse(topology, new Visitor<Node>() {
			...
			public void visit(final Node element) {
                for (final ExecutionNode en : element.getExecutionNodes()) {
                    final TaskStateTransitionEvent event = new TaskStateTransitionEvent(topology.topologyID,
                            en.getTaskDescriptor().taskID, transition);
